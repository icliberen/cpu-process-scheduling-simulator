...
def schedule_srtf(processes: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    while completed < n:
        ...
        ready.sort(key=lambda p: (remaining[p["pid"]], p["arrival"], p["index"]))
        cur = ready[0]
        pid = cur["pid"]

        if per_proc[pid]["start_time"] is None:
            per_proc[pid]["start_time"] = time
            log.append(f"t={time}: {pid} starts running (SRTF)")
        elif prev_pid is not None and prev_pid != pid:
            log.append(f"t={time}: {pid} preempts {prev_pid} (SRTF)")
        ...
        if remaining[pid] == 0:
            ...
            completed += 1
            log.append(f"t={time}: {pid} completes (SRTF)")
